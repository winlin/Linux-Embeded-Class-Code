...
static ssize_t firstchardevdriver_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if((0==pcdevp->led)&&(O_NONBLOCK&filp->f_flags)) {
		printk(KERN_ALERT "No Data and non-block. Return immediately \n");
		return 0;
	}
	
	//if pcdecp->led==0 ,no data to read ,add current process to wait wqeue
	wait_event_interruptible(pcdevp->wqh, (0!=pcdevp->led));
	printk(KERN_ALERT "pcdevp->led=1, and set it to 0.\n");
	pcdevp->led=0;
...
static int __init firstchardevdriver_init(void)
{
	...
	for(i=0;i<FIRSTCHARDEVDRIVER_COUNT;++i){
                //initialize the wait_queue
		init_waitqueue_head(&(firstchardevdriver_cdevp[i].wqh));
