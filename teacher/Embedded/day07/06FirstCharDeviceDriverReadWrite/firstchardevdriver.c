...
static ssize_t firstchardevdriver_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	char sbuf[ 50 ] = "Test string from kernel space to user space !";
	...
	if( 4 == count ) {
		...
		printk(KERN_INFO "FirstCharDevDriver: Kernel space write integer to user space: 0x%8x !\n", pcdevp->led);
	}
	else {
		if(copy_to_user( buf, sbuf, count )) {
			goto write_to_user_error;
		}
		printk(KERN_INFO "FirstCharDevDriver: Kernel space write string to user space: %s !\n", sbuf);
	}
...
static ssize_t firstchardevdriver_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if( 4 == count ) {
		...
		printk(KERN_INFO "FirstCharDevDriver: Kernel space read integer from user space: 0x%8x !\n", pcdevp->led);
	}
	else {
		if(copy_from_user( rbuf, buf, count )) {
			goto read_from_user_error;
		}
		printk(KERN_INFO "FirstCharDevDriver: Kernel space read string from user space: %s !\n", rbuf);
	}
