...
static ssize_t firstchardevdriver_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	down_interruptible( &(pcdevp->sem_read) );
	// critical section
	// up( &(pcdevp->sem_read) );
	printk(KERN_ALERT "FirstCharDevDriver: Success to get sem_read !\n");
...
static ssize_t firstchardevdriver_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	// critical section
	// up( &(pcdevp->sec_write) );
	printk(KERN_INFO "FirstCharDevDrier: Success to get sem_write !\n");
