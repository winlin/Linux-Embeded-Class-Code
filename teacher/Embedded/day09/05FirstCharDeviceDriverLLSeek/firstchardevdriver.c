...
static int firstchardevdriver_display_data(struct file *filp, struct firstchardevdriver_cdev *pcdevp)
{
	...
	printk(KERN_INFO "FirstCharDevDriver: Entry firstchardevdriver_display_data !\n");

	printk(KERN_ALERT "FirstCharDevDriver: current data length: %d !\n", pcdevp->dat_len);
	printk(KERN_ALERT "FirstCharDevDriver: current file pointer: %d !\n", (int)filp->f_pos);

	for( i = 0; i < MAX_DATA_LENGTH; i++ ) {
		printk("DAT[ %d] = %d\t", i, pcdevp->dat[ i ]);
		if( 0 == ((i + 1) % 5) ) {
			printk("\n");
		}
	}
...
static ssize_t firstchardevdriver_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if( pos >= pcdevp->dat_len ) {
		printk(KERN_ALERT "FirstCharDevDriver: file pointer is at the end !\n");
		return -EINVAL;
	}

	if( cnt > (pcdevp->dat_len -pos) ) {
		cnt = pcdevp->dat_len - pos;
	}

	if( copy_to_user( buf, &(pcdevp->dat[ pos ]), cnt ) ) {
		printk(KERN_ALERT "FirstCharDevDriver: Failure to copy data to user space !\n");
		return -EFAULT;
	}
...
static ssize_t firstchardevdriver_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if( MAX_DATA_LENGTH <= pcdevp->dat_len ) {
		printk(KERN_ALERT "FirstCharDevDriver: The buffer is full !\n");
		return -EINVAL;
	}

	if( (MAX_DATA_LENGTH - pcdevp->dat_len) < cnt ) {
		cnt = MAX_DATA_LENGTH - pcdevp->dat_len;
	}

	if( copy_from_user( &(pcdevp->dat[ pos ]), buf, cnt ) ) {
		printk(KERN_ALERT "FirstCharDevDriver: Failure to copy data from user space !\n");
		return -EFAULT;
	}
...
static loff_t firstchardevdriver_llseek(struct file *filp, loff_t off, int whence)
{
	...
	printk(KERN_ALERT "FirstCharDevDriver: Entry firstchardevdriver_llseek !\n");

	printk(KERN_ALERT "FirstCharDevDriver: old file pointer: %d !\n", (int)filp->f_pos);
	...
	case SEEK_CUR:
		...
		break;
	case SEEK_END:
		newpos = pcdevp->dat_len + off;
		break;
	default:
		return -EINVAL;
	}

	if( (0 > newpos) || (newpos > pcdevp->dat_len) ) {
		return -EINVAL;
	}

	filp->f_pos = newpos;
	printk(KERN_ALERT "FirstCharDevDrier: new file pointer: %d \n", (int)filp->f_pos);
