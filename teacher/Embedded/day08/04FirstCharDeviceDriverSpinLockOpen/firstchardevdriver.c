...
static int firstchardevdriver_open(struct inode *inode, struct file *filp)
{
	...
	if( ALLOW_OPEN_TIMES <= pcdevp->open_times ) {
		...
		printk(KERN_ALERT "FirstCharDevDriver: device can be open only %d times !\n", ALLOW_OPEN_TIMES);
		return -EBUSY;
	}
	pcdevp->open_times++;
	spin_unlock( &(pcdevp->lock) );
	printk(KERN_ALERT "FirstCharDevDriver: Success to open device !\n");
...
static int __init firstchardevdriver_init(void)
{
	...
	// intialize spinlock
	spin_lock_init( &(firstchardevdriver_cdevp->lock) );
	firstchardevdriver_cdevp->open_times = 0;
