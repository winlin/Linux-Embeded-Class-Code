...
static int devicemodeldemo_bus_match(struct device *dev, struct device_driver *drv)
{
	...
	printk(KERN_ALERT "DeviceModelDemo: Match Device & Driver: Dev Name: %s\tDrv Name: %s\n", dev->type->name, drv->name);
...
{
	printk(KERN_ALERT "DeviceModelDemo: Entry devicemodeldemo_drv_probe !\n");
...
{
	printk(KERN_ALERT "DeviceModelDemo: Entry devicemodeldemo_drv_remove !\n");
...
static struct device_driver devicemodeldemo_drv = {
	...
	.remove		= __devexit_p(devicemodeldemo_drv_remove),
};
...
static int __init devicemodeldemo_init(void)
{
	...
	if( 0 > ret ) {
		printk(KERN_ALERT "DeviceModelDemo: Failure to register device_driver: %s !\n", devicemodeldemo_drv.name);
		goto failure_driver_register;
	}
	printk(KERN_ALERT "DeviceModelDemo: Success to register device_driver: %s !\n", devicemodeldemo_drv.name);
