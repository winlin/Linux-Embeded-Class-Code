...
static void ledn_timer_handler(unsigned long data)
{
	...
	*gpncon &= 0xFFFFCFFF; // iowrite32(gpncon, 0xFFFFCFFFF);
	*gpncon |= 0x00001000;
	*gpnpud &= 0xFFFFCFFF;
	if( !pcdevp->led ) {
		pcdevp->led = 1;
...
static int __init firstchardevdriver_init(void)
{
	...
	if( !request_mem_region( IOMEM_GPN_BASE, IOMEM_GPN_SIZE, FIRSTCHARDEVDRIVER_NAME ) ) {
		printk(KERN_ALERT "FirstCharDevDriver: request mem_region for GPN error !\n");
	...
	firstchardevdriver_cdevp->gpn = (void __iomem *)0xF450830;
