/*由于我们是使用的内存来模拟的硬盘，是非机械类外部存储设备
 所以，对于request_queue操作的电梯算法，就没有实际的作用了
 该程序掩演示怎样把算法设置成noop
*/
...
{
	printk(KERN_ALERT "Entry virtualblockdevice_getgeo()\n");
...
static int __init virtualblockdevice_init(void)
{
	...
	oldelevator = virtualblockdevice_queue->elevator;
	if (IS_ERR_VALUE(elevator_init(virtualblockdevice_queue, "noop"))) {
		printk(KERN_ALERT "Fail to switch elevator to noop, contine to use old one\n");
	} else {
		printk(KERN_ALERT "Success to switch elevator to noop\n");
		elevator_exit(oldelevator);
	}
...
static void __exit virtualblockdevice_exit(void)
{
	...
	virtualblockdevice_disk = NULL;
	virtualblockdevice_queue = NULL;
}
