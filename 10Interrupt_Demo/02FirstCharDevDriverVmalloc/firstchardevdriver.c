...
static int __init firstchardevdriver_init(void)
{
	...
	if(IS_ERR(dev_class)){
		printk(KERN_INFO "FirstCharDevDriver: Can't  create one device class %s !\n",FIRSTCHARDEVDRIVER_CLASS_NAME);	
		ret = -1;
	...
	ret=firstchardevdriver_cdev_add(firstchardevdriver_cdevp,0);
	if(ret<0){
	...
	printk(KERN_ALERT "kernelmalloc = 0x%lx !\n",(unsigned long)kernelkmalloc);
...
static void __exit firstchardevdriver_exit(void)
{
	...
	cdev_del(&firstchardevdriver_cdevp->cdev);
